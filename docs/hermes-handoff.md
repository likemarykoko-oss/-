...
| 8 | SOUL.md, домашний канал, первый крон | ✅ |
| 9 | Финальная проверка, бэкап, отчёт | ✅ |
...
### Фаза 8 — личность, дом, крон

`SOUL.md` переписан ✅ (бэкап `SOUL.md.bak-20260822141909`): дружелюбно-неформальный
тон, на «ты», по-русски, имя оставлено Hermes, описаны реальные возможности
(Telegram голос/текст, веб-поиск, крон, память). Живой смоук пройден — ответ
получен в нужном тоне.

**Разобран ложный сигнал тревоги:** в чате с ботом обнаружился ответ с чужой
personoй ("AI creator для бизнеса", CRM/WhatsApp) — оказалось, что пользователь
включил в BotFather Threaded Mode (по стороннему туториалу), из-за чего каждое
сообщение открывает отдельную вкладку-тему в Telegram; странный ответ был
просто СТАРОЙ веткой, открытой раньше, а не живым ответом бота — `hermes-gateway`
её не обрабатывал (пустые логи за нужный период это подтвердили). Реальный
username бота подтверждён через `getMe`: `hermes_marykokobot` — совпадает
с тем, что записано в хендоффе.

**Первый крон поставлен ✅** обычной фразой боту в чате («Каждое утро в 9:00
присылай мне план на день и погоду») — бот сам создал задачу через свой
cronjob-тулсет. Подтверждено `hermes cron list`/`hermes cron status`:

```
fee5bed3a5a8 [active]  Утренний план и погода
Schedule: 0 9 * * *  Next run: 2026-08-23T09:00:00+03:00  Skills: google-workspace
```

Бот честно предупредил, что Google Calendar не подключён — будет собирать план
по доступным задачам без календаря (опциональное расширение, ставить не будем
без отдельного запроса пользователя). Домашний канал (`/sethome`) сделан ранее.

**Фаза 8 — полностью завершена ✅.**

### Фаза 9 — финальная проверка и отчёт ✅ ЗАВЕРШЕНО

1. `hermes doctor` / `hermes config check` / `hermes status` пройдены без новых ошибок.
   Все "not configured"/"not logged in" пункты (Qwen, MiniMax OAuth, xAI OAuth, Z.AI, Kimi,
   StepFun, DeepInfra, Firecrawl, Tavily, Browserbase, GitHub, Anthropic) — сознательно
   не используемые провайдеры, не баг. Primary (Codex) и Fallback 3 (Nous Portal) подтверждены
   `✓ logged in` в `config check`. `Nous Tool Gateway: no usable paid credits` — это отдельный
   платный managed-сервис Nous, не используется (свои бесплатные web/voice инструменты работают).
2. `systemctl restart hermes-gateway` → `active`, гейтвей перезапущен чисто.
3. Финальные живые смоуки в `@hermes_marykokobot` — все три пройдены:
   - текст → дружелюбный ответ в новом тоне;
   - голосовое → распознано, дан ответ голосом (погода в Набережных Челнах);
   - «найди в интернете сколько стоит биткоин» → сходил в несколько источников (CoinGecko,
     Binance, Coinbase, Coinpaprika), дал ответ с разбросом цен по биржам и ссылкой.
4. `hermes backup` выполнен: `/root/hermes-backup-2026-08-22-143920.zip`, 3367 файлов,
   289.9 МБ → 91.7 МБ сжато. Скачан пользователем на iPad через Termius SFTP
   (`Connect via SFTP` в контекстном меню хоста → `/root/`).
   **Правило на будущее: перед каждым `hermes update` — сначала `hermes backup`.**
5. Опциональные расширения (Google Workspace/календарь, image-gen, CloakBrowser/yt-dlp/NotebookLM)
   не устанавливались — предложены пользователю, ставить только по отдельному запросу.

**Фаза 9 — полностью завершена ✅. Установка Hermes закрыта целиком (фазы 0–9).**
...
## Итоговое состояние (на 2026-08-22)

- **Сервер:** `216.57.111.124` (hermes, Timeweb), доступ только по SSH-ключу `hermes-vps`, UFW пропускает только `22/tcp`, fail2ban включён.
- **Бот:** Telegram `@hermes_marykokobot`, домашний канал настроен, отвечает текстом и голосом.
- **Мозг:** Codex (primary) → OpenRouter → Gonka → Nous Portal (fallback-цепочка), все звенья живые.
- **Возможности:** веб-поиск/извлечение (встроенные, без внешних ключей), голос (Groq+local STT, edge-tts), крон (1 активная задача — утренняя сводка в 9:00 МСК), память, личность настроена под пользователя.
- **Известное ограничение:** browser automation (Playwright) не работает — Ubuntu 26.04 официально не поддерживается Playwright 1.58.2; не блокирует основной функционал.
- **Бэкапы:** есть на сервере (`/root/hermes-backup-*.zip`) и скачаны локально пользователем.
- **Не установлено (по желанию в будущем):** Google Workspace/календарь, image-gen, CloakBrowser/yt-dlp/NotebookLM.
