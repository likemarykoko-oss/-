...
Интерактивная, зависит от того, что у тебя есть. Варианты по приоритету:

**Nous Portal** (родной, есть бесплатные модели):
```bash
hermes portal
```
Напечатает URL — открой на своей машине, залогинься. ⚠ Без списания кредитов
работают только модели с тегом `:free`.

**OpenRouter** (ключ с openrouter.ai → Keys):
```bash
...
**Codex / ChatGPT Plus** — сначала включи device-авторизацию в chatgpt.com →
Settings → Security, иначе код не примут. Детали: `skills/hermes-vps-setup/references/providers.md` §3.
...
```
Без ответа с PONG дальше не иди — без мозга остальные фазы не проверить.
