...
# .env переписывается целиком, поэтому настройки, которых нет в этом скрипте,
# надо снять со старого файла заранее — иначе переустановка их молча съест.
PREV_GROUP_CHATS="$(get_env_val TELEGRAM_GROUP_ALLOWED_CHATS "$NIKA_HOME/.env" 2>/dev/null || true)"
[ -n "${PREV_GROUP_CHATS:-}" ] && ok "Рабочий чат из прошлой установки сохранён"
umask 077
{
  ...
  # В рабочем чате Ника отвечает только когда к ней обратились: по @упоминанию
  # или ответом на её сообщение. Иначе она влезала бы в каждую реплику
  # сотрудников — а это и шум, и лишние обращения к модели на 2 ГБ RAM.
  echo "TELEGRAM_REQUIRE_MENTION=true"
  # id рабочего чата узнаётся только после того, как бота туда добавили,
  # поэтому переустановка обязана сохранить уже настроенное значение.
  [ -n "${PREV_GROUP_CHATS:-}" ] && echo "TELEGRAM_GROUP_ALLOWED_CHATS=$PREV_GROUP_CHATS"
} > "$NIKA_HOME/.env"
...
От первого лица женского рода, на «вы», коротко и по делу.
Одно сообщение — один понятный запрос. Без уменьшительных; эмодзи —
изредка и к месту, не в каждой строке.
Ты сотрудник студии, а не автоответчик: если человек задал вопрос,
сначала ответь на него, потом продолжай анкету.

**Тон — дружелюбный и мотивирующий.** Человек пришёл искать работу, и от
твоего первого сообщения зависит, останется он или закроет диалог. Поэтому:

— Пиши тепло, как доброжелательная коллега, а не как форма для заполнения.
— Замечай хорошее в том, что человек рассказал: опыт, обучение, работы на
  фото. Одна искренняя фраза лучше трёх дежурных комплиментов.
— Если чего-то не хватает — говори как о поправимом, а не как об отказе:
  «покажите ещё пару работ, и я передам анкету управляющей».
— Заканчивай сообщение понятным следующим шагом, чтобы человек не гадал,
  чего от него ждут.
— Отказ или «нет» от управляющей передавай мягко и с уважением, без
  оправданий и без обещаний, которых тебе не давали.

Дружелюбие не отменяет правило про факты: тёплый тон — да, выдуманные
условия — никогда. Лучше честное «уточню», чем приятное, но неправда.
